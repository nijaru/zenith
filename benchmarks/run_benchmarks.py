...
class BenchmarkRunner:
    ...
    async def run_benchmark(self, framework: str, port: int, test: dict, duration: int = 10):
    ...
    def print_summary(self):
        ...
        for test in TESTS:
            ...
            print("-" * 40)
            print(f"{'Framework':<12} {'RPS':>10} {'P50 (ms)':>10} {'P95 (ms)':>10} {'Memory Δ':>10}")
            print("-" * 40)
            
            for framework in FRAMEWORKS:
                if test["name"] in self.results[framework]:
                    r = self.results[framework][test["name"]]
                    print(f"{framework:<12} {r['requests_per_second']:>10.1f} "
