...
class TestRouterIntegration:
    ...
    async def test_query_parameters(self):
        ...
        @app.get("/search")
        async def search(q: str, limit: int = 10, sort: str = "name", active: bool = True):
            return {
                "query": q,
                "limit": limit,
                "sort": sort,
    ...
    async def test_authentication_dependency(self):
        ...
        app = Zenith()
        configure_auth(app, secret_key="test-secret-key-that-is-long-enough-for-jwt-signing")
    ...
    async def test_response_types(self):
        ...
        @app.get("/model")
        async def return_model() -> TestUser:
            return TestUser(name="John", email="john@test.com", age=30)
