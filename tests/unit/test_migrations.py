...
class TestMigrationManager:
    ...
    def test_migration_manager_init(self, mock_database, temp_migrations_dir):
        ...
        assert (
            manager.alembic_cfg.get_main_option("sqlalchemy.url") == mock_database.url
        )
    ...
    def test_init_migrations(self, mock_command, mock_database, temp_migrations_dir):
        ...
        mock_command.init.assert_called_once_with(
            manager.alembic_cfg, manager.script_location
        )

    @patch("zenith.db.migrations.command")
    def test_init_migrations_already_exists(
        self, mock_command, mock_database, temp_migrations_dir
    ):
    ...
    @patch("zenith.db.migrations.command")
    def test_create_migration_autogenerate(
        self, mock_command, mock_database, temp_migrations_dir
    ):
        ...
        mock_command.revision.assert_called_once_with(
            manager.alembic_cfg, message="Add user table", autogenerate=True
        )

    @patch("zenith.db.migrations.command")
    def test_create_migration_manual(
        self, mock_command, mock_database, temp_migrations_dir
    ):
        ...
        mock_command.revision.assert_called_once_with(
            manager.alembic_cfg, message="Custom migration"
        )
    ...
    @patch("zenith.db.migrations.command")
    def test_upgrade_database_error(
        self, mock_command, mock_database, temp_migrations_dir
    ):
    ...
    @patch("zenith.db.migrations.command")
    def test_downgrade_database_error(
        self, mock_command, mock_database, temp_migrations_dir
    ):
    ...
    def test_current_revision(
        self, mock_script_dir, mock_create_engine, mock_database, temp_migrations_dir
    ):
    ...
    def test_migration_history(
        self, mock_script_dir, mock_database, temp_migrations_dir
    ):
    ...
    def test_status(self, mock_database, temp_migrations_dir):
        ...
        with (
            patch.object(manager, "current_revision", return_value="abc123"),
            patch.object(
                manager,
                "migration_history",
                return_value=[
