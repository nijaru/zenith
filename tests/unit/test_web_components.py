...
from zenith.web.files import FileUploadConfig, FileUploader, UploadedFile, FileUploadError
from zenith.web.health import HealthManager, HealthCheck, HealthStatus, add_health_routes
from zenith.web.responses import (
    success_response, error_response, file_download_response,
    paginated_response, redirect_response
)
...
class TestFileUploads:
    ...
    async def test_file_upload_basic(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            @app.post("/upload")
            async def upload_file(file = File("file", config), current_user = Auth(required=True)):
                return {
            ...
            async with TestClient(app) as client:
                ...
                response = await client._client.post("/upload", files=files)
    ...
    async def test_file_upload_validation(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            async with TestClient(app) as client:
                ...
                response = await client._client.post("/upload", files=files)
                assert response.status_code == 500  # Should be caught as FileUploadError
...
class TestHealthChecks:
    ...
    async def test_health_manager_basic(self):
        ...
        manager.add_simple_check("test", test_check, timeout=1.0)
    ...
    async def test_health_check_timeout(self):
        ...
        manager.add_simple_check("slow", slow_check, timeout=0.1, critical=True)
...
class TestResponseUtilities:
    ...
    def test_error_response(self):
        ...
        response = error_response("Invalid input", 400, "VALIDATION_ERROR", {"field": "email"})
