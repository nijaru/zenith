...
class TestMiddlewarePerformance:
    ...
    async def test_cors_middleware_performance(self, base_app):
        ...
        # Calculate overhead
        get_overhead = (
            (cors_metrics["get_avg"] - base_metrics["get_avg"])
            / base_metrics["get_avg"]
        ) * 100
        post_overhead = (
            (cors_metrics["post_avg"] - base_metrics["post_avg"])
            / base_metrics["post_avg"]
        ) * 100

        print("\nCORS Middleware Performance:")
        print(
            f"  Base GET:     {base_metrics['get_avg'] * 1000:.2f}ms ({base_metrics['get_rps']:.0f} req/s)"
        )
        print(
            f"  CORS GET:     {cors_metrics['get_avg'] * 1000:.2f}ms ({cors_metrics['get_rps']:.0f} req/s)"
        )
        print(f"  GET overhead: {get_overhead:.1f}%")
        print(
            f"  Base POST:    {base_metrics['post_avg'] * 1000:.2f}ms ({base_metrics['post_rps']:.0f} req/s)"
        )
        print(
            f"  CORS POST:    {cors_metrics['post_avg'] * 1000:.2f}ms ({cors_metrics['post_rps']:.0f} req/s)"
        )
    ...
    async def test_compression_middleware_performance(self, base_app):
        ...
        compression_app = Zenith(
            debug=False,
            middleware=[Middleware(CompressionMiddleware, minimum_size=500)],
        )
        ...
        get_overhead = (
            (compression_metrics["get_avg"] - base_metrics["get_avg"])
            / base_metrics["get_avg"]
        ) * 100
    ...
    async def test_security_headers_middleware_performance(self, base_app):
        ...
        security_app = Zenith(
            debug=False,
            middleware=[Middleware(SecurityHeadersMiddleware, config=security_config)],
        )
        ...
        get_overhead = (
            (security_metrics["get_avg"] - base_metrics["get_avg"])
            / base_metrics["get_avg"]
        ) * 100
    ...
    async def test_request_logging_middleware_performance(self, base_app):
        ...
        logging_app = Zenith(
            ...
            middleware=[
                Middleware(
                    RequestLoggingMiddleware, exclude_paths=[], include_body=False
                )
            ],
        )
        ...
        get_overhead = (
            (logging_metrics["get_avg"] - base_metrics["get_avg"])
            / base_metrics["get_avg"]
        ) * 100
        post_overhead = (
            (logging_metrics["post_avg"] - base_metrics["post_avg"])
            / base_metrics["post_avg"]
        ) * 100
    ...
    async def test_rate_limit_middleware_performance(self, base_app):
        ...
        rate_limit_metrics = await self.measure_app_performance(
            rate_limit_app, iterations=100
        )

        get_overhead = (
            (rate_limit_metrics["get_avg"] - base_metrics["get_avg"])
            / base_metrics["get_avg"]
        ) * 100
    ...
    async def test_middleware_stack_performance(self):
        ...
        # Compare performance
        minimal_metrics = await self.measure_app_performance(
            minimal_app, iterations=300
        )
        full_metrics = await self.measure_app_performance(full_app, iterations=300)

        total_overhead = (
            (full_metrics["get_avg"] - minimal_metrics["get_avg"])
            / minimal_metrics["get_avg"]
        ) * 100

        print("\nFull Middleware Stack Performance:")
        print(
            f"  Minimal:       {minimal_metrics['get_avg'] * 1000:.2f}ms ({minimal_metrics['get_rps']:.0f} req/s)"
        )
        print(
            f"  Full stack:    {full_metrics['get_avg'] * 1000:.2f}ms ({full_metrics['get_rps']:.0f} req/s)"
        )
        ...
        assert (
            total_overhead < 800
        )  # < 800% total overhead (TestClient adds significant overhead)
