...
async def submit_email_job(request: EmailRequest) -> dict:
    ...
    job_id = await send_email.delay(request.to, request.subject, request.body)
...
async def submit_image_job(request: ImageRequest) -> dict:
    ...
    # Use the .delay() method attached by the @job decorator
    job_id = await process_image.delay(request.image_url, request.sizes)
...
async def schedule_email_later(request: EmailRequest, delay_minutes: int = 5) -> dict:
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        if command == "worker":
            ...
            async def run_all():
                await asyncio.gather(run_worker(), run_scheduler())
