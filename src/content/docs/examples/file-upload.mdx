...
Zenith v0.3.1 includes an enhanced File dependency with intuitive validation:

```python
from zenith import File, UploadedFile
from zenith.core.dependencies import IMAGE_TYPES, DOCUMENT_TYPES, AUDIO_TYPES, VIDEO_TYPES

# String-based size limits (more readable than raw bytes)
@app.post("/avatar")
async def upload_avatar(
    avatar: UploadedFile = File(
        max_size="5MB",           # Instead of max_size=5*1024*1024
        allowed_types=IMAGE_TYPES, # Pre-defined type constants
...
# Multiple file types with flexible sizing
@app.post("/media")
async def upload_media(
    files: List[UploadedFile] = File(
        max_size="100MB",         # Supports KB, MB, GB
        allowed_types=IMAGE_TYPES + VIDEO_TYPES + AUDIO_TYPES,
        allowed_extensions=[".jpg", ".png", ".mp4", ".mp3", ".pdf"]
    )
):
    """Upload media files with comprehensive validation."""
...
from zenith import Service, Inject, UploadFile
...
from zenith import Router, UploadedFile, File, Form, Inject, HTTPException, BackgroundTasks
from zenith.core.dependencies import IMAGE_TYPES, DOCUMENT_TYPES, AUDIO_TYPES
...
async def upload_multiple_files(
    files: List[UploadedFile] = File(
        max_size="50MB",
        allowed_types=IMAGE_TYPES + DOCUMENT_TYPES + AUDIO_TYPES,
        allowed_extensions=[".jpg", ".png", ".pdf", ".docx", ".mp3", ".wav"],
