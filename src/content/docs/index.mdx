---
title: ⚡ Zenith Framework
description: Modern Python web framework with clean architecture and exceptional performance
template: splash
hero:
  tagline: Build production-ready APIs with clean architecture and exceptional developer experience
  actions:
    - text: Get Started
      link: /introduction/
      icon: right-arrow
      variant: primary
    - text: View on GitHub
      link: https://github.com/nijaru/zenith
      icon: github
---

import { Card, CardGrid } from '@astrojs/starlight/components';

## Build Modern Python APIs

<CardGrid stagger>
	<Card title="Clean Architecture" icon="puzzle">
		Organize business logic with Phoenix-inspired contexts. Keep your domain logic independent of web concerns.
	</Card>
	<Card title="Type-Safe by Default" icon="shield">
		Full type hints with Pydantic validation. Catch errors at development time with complete IDE support.
	</Card>
	<Card title="SQLModel Integration" icon="database">
		One model for everything - API validation, database tables, and documentation. No more duplication.
	</Card>
	<Card title="Production Ready" icon="rocket">
		Built-in middleware for security, CORS, rate limiting, compression, monitoring, and health checks.
	</Card>
</CardGrid>

## Quick Start

```bash
# Install Zenith
pip install zenith-web

# Create a new project
zen new my-api --template api

# Start development server
zen server --reload
```

## Why Developers Choose Zenith

<CardGrid>
	<Card title="Developer Experience" icon="heart">
		Modern CLI tools, hot reload, auto-documentation, and comprehensive testing utilities make development a joy.
	</Card>
	<Card title="Performance First" icon="rocket">
		Built on Starlette with modern async patterns. Leverages Python 3.12+ features for optimal performance.
	</Card>
	<Card title="Battle Tested" icon="shield">
		100% test coverage with 293 tests. Zero memory leaks with bounded caches and automatic cleanup.
	</Card>
	<Card title="Framework Agnostic" icon="puzzle">
		Serve any SPA framework with one command. React, Vue, Angular, SolidJS - we've got you covered.
	</Card>
</CardGrid>
