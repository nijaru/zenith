...
Zenith provides `ZenithModel` - an enhanced base class that adds chainable queries, automatic session management, and superior developer experience to SQLModel.

<Aside type="note">
  **ZenithModel** combines SQLModel's type safety with intuitive query patterns for ultimate productivity.
</Aside>
...
### ZenithModel Approach (Modern DX)

```python
# ZenithModel provides intuitive patterns with automatic session management
...
# Intuitive chainable queries - no manual session needed!
async def get_users():
...
class User(ZenithModel, table=True):
    # Enhanced methods like where(), find(), create() provided automatically
    id: int | None = Field(primary_key=True)
    name: str
    email: str
```
