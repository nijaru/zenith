...
class TestPerformance:
    ...
    async def test_route_execution_speed(self):
        ...
        config = Config(
            debug=False, secret_key="test-secret-key-for-performance-testing-32chars"
        )
    ...
    async def test_json_serialization_performance(self):
        ...
        large_data = {
            "items": [
                {"id": i, "name": f"Item {i}", "value": i * 1.5} for i in range(1000)
            ]
        }
    ...
    async def test_concurrent_request_handling(self):
        ...
        async with TestClient(app) as client:
            ...
            tasks = [client.get(f"/concurrent/{0.1}") for _ in range(10)]
