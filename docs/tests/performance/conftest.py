...
def performance_config():
    ...
    return {
        "iterations": {"basic": 200, "middleware": 100, "load": 50, "memory": 500},
        "thresholds": {
            "response_time_ms": 20,
...
def benchmark():
    ...
    class Benchmark:
        ...
        async def time_async_function(
            self, func, *args, iterations=100, warmup=10, **kwargs
        ):
...
def assert_response_time(
    actual_time_ms: float, max_time_ms: float, test_name: str = "test"
):
...
def assert_overhead(
    baseline: float, actual: float, max_overhead_pct: float, test_name: str = "test"
):
