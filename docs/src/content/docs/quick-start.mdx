...
```python
from zenith import Zenith, HTTPException
...
<Aside type="tip">
  **For production deployment**, use `production=True` to enable the full middleware stack:
  ```python
  app = Zenith(production=True)
  ```
  This enables security headers, rate limiting, compression, and request tracking automatically.
</Aside>
