...
async def list_posts(
    ...
    if published is not None:
        query = await Post.where(published=published)
    else:
        query = await Post.where()  # All posts
...
async def publish_post(post_id: int, user=Auth):
    ...
    return {"message": "Post published", "post": post.to_dict()}
...
# Serialization
data = post.to_dict()                   # Convert to dictionary
post = Post.from_dict(data)             # Create from dictionary
```
...
async def search_posts(q: str):
    ...
    return {"query": q, "posts": [p.to_dict() for p in posts]}
```
