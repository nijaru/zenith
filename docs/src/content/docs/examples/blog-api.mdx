...
The authentication service handles user registration, login, and token generation. It separates business logic from route handlers for better testing and reusability:
...
class AuthService(Service):
    ...
    Service classes automatically get database access via self.db
    """
    ...
    async def register_user(self, user_data: UserCreate) -> dict:
        ...
        user = User(
            ...
            # hash_password() uses Argon2 for secure password storage
        ...
        await self.db.commit()   # Persist to database
        await self.db.refresh(user)  # Get generated ID and defaults

        # Step 5: Generate JWT token for immediate login
        token = create_access_token({
            "sub": user.email,    # JWT subject (primary identifier)
            "user_id": user.id,   # Our internal user ID
...
class PostsService(Service):
    ...
    async def get_posts(
        ...
        """
        Get posts with flexible filtering and pagination.
        ...
        # Each filter is only applied if provided
        if status:
        ...
        if featured_only:
            conditions.append(Post.is_featured == True)  # noqa: E712

        if search:
            # For text search, use ILIKE (case-insensitive LIKE)
            # % wildcards match any characters
            search_term = f"%{search}%"
            conditions.append(
                # or_() combines conditions with OR logic
                or_(
                    Post.title.ilike(search_term),     # Search in title
                    Post.content.ilike(search_term),   # Search in content
                    Post.excerpt.ilike(search_term)    # Search in excerpt
                )
            )
        ...
        statement = (
            statement
            .order_by(Post.published_at.desc())  # Newest first
    ...
    def generate_slug(self, title: str) -> str:
        ...
        """
        # Step 1: Remove special characters, keep only letters, numbers, spaces, hyphens
        slug = re.sub(r'[^\w\s-]', '', title).strip().lower()
        # Step 2: Replace multiple spaces/hyphens with single hyphen
        slug = re.sub(r'[-\s]+', '-', slug)
        # Step 3: Limit length for URL friendliness
        return slug[:50]
    ...
    async def ensure_unique_slug(self, slug: str, exclude_id: Optional[int] = None) -> str:
        ...
        Example: "my-post" exists -> returns "my-post-1"
        """
        original_slug = slug
        counter = 1
        ...
        while True:
            ...
            statement = select(Post).where(Post.slug == slug)
            if exclude_id:
                # When updating, exclude current post from check
            ...
            # Slug exists, try with counter: my-post-1, my-post-2, etc.
            slug = f"{original_slug}-{counter}"
            counter += 1

    def generate_excerpt(self, content: str, length: int = 200) -> str:
        """
        Generate excerpt from content for preview.
        ...
        if len(text) <= length:
            return text  # Content is already short enough

        # Truncate at word boundary to avoid cutting words
        # rsplit(' ', 1)[0] removes the last partial word
...
async def get_current_user_info(
    current_user = Inject(get_current_user)
):
...
```python
# app/routes/posts.py
from zenith import Router, Inject, HTTPException, Query
from zenith.auth import get_current_user
...
from zenith import Router, UploadFile, File, Inject, HTTPException
from zenith.auth import get_current_user
from app.models.user import User
import os
import uuid
