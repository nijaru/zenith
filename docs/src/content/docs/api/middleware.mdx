...
### When to Use Specialized Middleware

**File Operations:**
- Use `ZeroCopyStreamingMiddleware` for applications handling large file uploads/downloads

**Database-Heavy Applications:**
- Use `DatabaseConnectionReuseMiddleware` for applications with frequent database operations

**Real-Time Applications:**
- Use `ConcurrentWebSocketMiddleware` for applications with intensive WebSocket usage
- Use `ServerSentEventsBackpressureMiddleware` for applications with many concurrent SSE connections

### ZeroCopyStreamingMiddleware

Memory-efficient file operations with zero-copy optimization:
...
app.add_middleware(ZeroCopyStreamingMiddleware, {
    ...
    "max_chunk_size": 8192,  # 8KB chunks for optimal streaming
    "enable_streaming_validation": True,
    
    # Paths that should use zero-copy streaming
    "streaming_paths": ["/upload", "/download", "/files"],
    
    # Upload directory configuration
    "upload_directory": "/tmp/app_uploads"
})
```

**Use case**: Applications handling large file uploads or downloads (>1MB files).
**Memory reduction**: 40-60% less memory usage for large file transfers.


### DatabaseConnectionReuseMiddleware

Optimizes database performance by reusing connections across request lifecycle:

```python
from zenith.middleware import DatabaseConnectionReuseMiddleware

app.add_middleware(DatabaseConnectionReuseMiddleware, {
    # Database URL (AsyncPG or SQLAlchemy async)
...
**Use case**: Applications with frequent database queries (>10 queries per request).
**Performance improvement**: 15-25% faster database operations.

### ConcurrentWebSocketMiddleware

Enhances WebSocket performance with concurrent message processing:
...
app.add_middleware(ConcurrentWebSocketMiddleware, {
    ...
    "queue_size": 100,  # Message queue size per connection
    "process_concurrently": True  # Enable concurrent processing
})
```

**Use case**: Applications with high WebSocket message volume (chat apps, real-time dashboards).
**Throughput improvement**: 15-25% better WebSocket performance under load.
...
app.add_middleware(ServerSentEventsBackpressureMiddleware, {
    ...
    "max_queue_size": 1000,  # Max queued events per connection
    "drop_policy": "oldest",  # "oldest" or "newest" when queue full
...
**Use case**: Applications with many concurrent SSE connections (live feeds, notifications).
**Scaling improvement**: 10x more concurrent SSE connections without memory issues.
...
app.add_middleware(ZeroCopyStreamingMiddleware)       # File operations (if needed)
app.add_middleware(DatabaseConnectionReuseMiddleware) # Database optimization (if needed)
app.add_middleware(ConcurrentWebSocketMiddleware)     # WebSocket performance (if needed)
app.add_middleware(ServerSentEventsBackpressureMiddleware) # SSE scaling (if needed)
