...
class UserService(Service):
    ...
    def __init__(self):
        super().__init__()  # Sets up all the magic properties
        # That's it! Database session is automatically available as self.db
        # No need to inject database or manage connections
...
class UserService(Service):
    ...
    def __init__(
        self,
        # Inject() automatically provides service instances
        email_service: EmailService = Inject(),
        storage: StorageService = Inject()
        # Add more as needed:
        ...
        Zenith creates these services for you!
        No need to manually instantiate or manage them.
        """

        super().__init__()  # Important: Sets up Service base features

        # Store injected services for later use
...
async def create_user(
    ...
    users: UserService = Inject()  # ✨ Magic dependency injection!
) -> User:
    """Create a new user.
...
async def get_user(
    ...
    users: UserService = Inject()  # Injected service
) -> User:
    """Get user by ID.
...
async def create_order(
    ...
    orders: OrderService = Inject(),                # Order logic
    inventory: InventoryService = Inject(),         # Stock management
    payments: PaymentService = Inject()            # Payment processing
) -> Order:
