...
<Aside type="tip">
  Zenith achieves **37,000+ requests/second** on JSON endpoints with optimized request handling.
</Aside>

```python
# Performance baseline (measured on M3 Max, Python 3.13)
# - Simple endpoints: ~37,000 req/s (v0.0.13 with handler caching)
# - With full middleware: ~9,000 req/s (production mode)
