...
@app.get("/users/search")
async def search_users_safe(query: str, session: AsyncSession = Depends(get_session)):
    # Use parameterized queries
    stmt = select(User).where(User.email == query)
    result = await session.exec(stmt)
    return result.all()
...
class SecurityHeadersMiddleware:
    ...
    def _get_csp(self) -> str:
        ...
        directives = {
            "default-src": ["'self'"],
            "script-src": ["'self'", "'unsafe-inline'"] if settings.DEBUG else ["'self'"],
            "style-src": ["'self'", "'unsafe-inline'"],
            "img-src": ["'self'", "data:", "https:"],
            "font-src": ["'self'", "data:"],
            "connect-src": ["'self'"],
...
<Card title="Performance Guide" icon="arrow-right">
  Learn about performance optimization [Performance Guide →](/guides/performance)
</Card>
