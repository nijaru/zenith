...
<Aside type="tip">
  **Zero Configuration in v0.0.1+**: Zenith automatically configures your database connection based on environment. SQLite for development, production databases via DATABASE_URL.
</Aside>
...
# Includes serialization (model_dump())
...
<Aside type="tip">
  **Seamless Chaining (v0.0.5+):**
  `.where()` returns a QueryBuilder immediately (no await needed), enabling clean single-line chaining like `await User.where(email=email).first()`. The session is fetched lazily when you call terminal methods like `.first()`, `.all()`, `.count()`, or `.exists()`.
</Aside>
