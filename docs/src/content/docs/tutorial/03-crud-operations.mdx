...
<Aside type="tip">
  **Why CRUD matters**: These four operations form the backbone of 90% of web APIs. The difference is that with traditional ORMs, you write 200+ lines of boilerplate per model. With ZenithModel, you get more features in 40 lines!
</Aside>

## ZenithModel vs Traditional ORMs

Before we start, let's see why Zenith's patterns are so much cleaner than traditional approaches:

<Card title="Traditional SQLAlchemy - Complex and Error-Prone" icon="down-caret">
```python
...
**Why Zenith is better:**
- 📝 **Readable**: `User.where(active=True)` vs `select(User).where(User.active == True)`
- 🔗 **Chainable**: Method chaining flows naturally
- 🚫 **Less boilerplate**: No manual session management in business logic
...
<Card title="Continue to Part 4" icon="arrow-right">
  Ready to add authentication? [Continue to Authentication →](/tutorial/04-authentication)
</Card>
