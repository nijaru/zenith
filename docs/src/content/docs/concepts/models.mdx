...
Zenith enhances SQLModel with better developer experience through the `Model` base class, providing enhanced configuration and cleaner patterns for production applications.

<Aside type="note">
  **Zenith Model** is a clean alias for SQLModel with sensible defaults and enhanced configuration for better developer experience.
</Aside>

## Why Enhanced SQLModel?

### Traditional SQLModel Approach

```python
# Standard SQLModel requires manual configuration
...
### Zenith Approach (Enhanced DX)

```python
# Zenith Model provides sensible defaults automatically
...
class Comment(Model, table=True):
    """Comment model."""
...
```python
from zenith import Service, Session
