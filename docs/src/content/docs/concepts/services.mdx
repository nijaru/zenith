...
### Standalone Service Usage (Outside DI)
...
<Aside type="tip">
  **When to use `Service.create()`:**
  - Helper functions that need business logic
  - Custom middleware
  - Background jobs and scheduled tasks
  - CLI commands and scripts
  - Testing (when not using `TestService`)

  **When to use `Inject()`:**
  - Route handlers (preferred for DI container benefits)
  - When you need container-managed services
  - When you want singleton behavior
</Aside>
...
This pattern was introduced to solve [Issue #2](https://github.com/anthropics/zenith/issues/2) from real-world usage.
...
async def test_user_creation():
    ...
    async with TestService(UserService) as users:
        ...
        assert user.password_hash.startswith("$argon2")  # Argon2 format
