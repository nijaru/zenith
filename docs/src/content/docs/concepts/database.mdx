...
from zenith.core import DB  # Dependency injection shortcut
...
# Define models with ZenithModel
...
# Use in routes
@app.post("/users")
async def create_user(user: User, db=DB):
    # Database session is managed automatically
