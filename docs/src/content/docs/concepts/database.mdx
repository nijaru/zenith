...
from zenith.core import DB  # Enhanced dependency injection shortcut
from zenith.db import ZenithModel  # Rails-like model with built-in methods
...
# Define models with ZenithModel for Rails-like experience
...
# Use in routes with enhanced dependency injection
@app.post("/users")
async def create_user(user: User, db=DB):  # Clean DI - no Depends()!
    # ZenithModel automatically uses the request-scoped session
    new_user = await User.create(name=user.name, email=user.email)
    return new_user

@app.get("/users")
async def get_users():
    # Rails-like chainable queries
...
async def list_users(db=DB):
    # Use ZenithModel for Rails-like queries
