...
async def get_users(request):
    # Step 1: Manually extract query parameter from the request object
    page = request.query_params.get('page', '1')  # Default to string '1'

    # Step 2: Try to convert string to integer (can fail!)
    try:
        page = int(page)
        # Step 3: Validate the converted value is within bounds
        if page < 1:
            return JSONResponse({"error": "Invalid page"}, 400)
    except ValueError:
        # Step 4: Handle conversion errors when user sends "abc" instead of number
        return JSONResponse({"error": "Page must be integer"}, 400)

    # Step 5: Repeat this painful process for EVERY parameter...
    limit = request.query_params.get('limit', '10')
    try:
        limit = int(limit)
        if limit < 1 or limit > 100:  # More validation logic to maintain
            return JSONResponse({"error": "Limit must be 1-100"}, 400)
    except ValueError:
        return JSONResponse({"error": "Limit must be integer"}, 400)

    # ... Imagine doing this for 10+ parameters! 🤯

# After: Zenith automatic validation - clean and type-safe
@app.get("/users")
async def get_users(
    page: int = 1,      # Zenith knows this is an integer from query string
    limit: int = 10     # Type hint = automatic validation!
):
    # That's it! Zenith automatically:
...
```python
# Traditional approach - multiple HTTP methods on same route
# Route: /users/<user_id>
