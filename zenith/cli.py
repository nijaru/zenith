...
@main.command()
@click.option("--template", type=click.Choice(["heroku", "railway", "docker", "local", "aws", "gcp"]), default="local", help="Platform template")
@click.option("--output", "-o", default=".env", help="Output file (default: .env)")
@click.option("--force", is_flag=True, help="Overwrite existing file")
def env(template: str, output: str, force: bool):
    """Generate platform-specific environment templates.
    ...
    Platform templates:
      ...
      aws      - AWS deployment (ECS/Lambda)
      gcp      - Google Cloud Platform deployment

    Examples:
      zen env --template heroku      # Heroku-specific variables
      zen env --template railway     # Railway-specific variables
      zen env --template docker      # Docker environment template
    """
    from datetime import datetime
    ...
    if output_path.exists() and not force:
        ...
        click.echo("   Use --force to overwrite existing file")
        click.echo("   Or specify a different output file with --output")
        sys.exit(1)

    # Platform-specific templates
    templates = {
        "local": _generate_local_env_template,
        "heroku": _generate_heroku_env_template,
        "railway": _generate_railway_env_template,
        "docker": _generate_docker_env_template,
        "aws": _generate_aws_env_template,
        "gcp": _generate_gcp_env_template,
    }

    template_generator = templates[template]
    content = template_generator(timestamp)

    try:
        output_path.write_text(content)
        click.echo(f"✅ {template.title()} environment template written to {output}")

        # Platform-specific tips
        tips = {
            "heroku": [
                "Set environment variables: heroku config:set KEY=value",
                "Add DATABASE_URL will be provided automatically by Heroku Postgres",
                "Configure buildpacks: heroku buildpacks:add heroku/python"
            ],
            "railway": [
                "Environment variables are set in Railway dashboard",
                "DATABASE_URL will be provided automatically by Railway Postgres",
                "Use Railway CLI: railway variables set KEY=value"
            ],
            "docker": [
                "Use docker-compose.yml or Dockerfile ENV commands",
                "Consider using Docker secrets for sensitive data",
                "Mount .env file or use --env-file flag"
            ],
            "aws": [
                "Set environment variables in ECS task definition or Lambda config",
                "Use AWS Secrets Manager for sensitive values",
                "Configure IAM roles for service access"
            ],
            "gcp": [
                "Set environment variables in Cloud Run or App Engine config",
                "Use Secret Manager for sensitive values",
                "Configure service accounts for API access"
            ]
        }

        if template in tips:
            click.echo(f"\n💡 {template.title()} deployment tips:")
...
def _generate_local_env_template(timestamp: str) -> str:
    """Generate local development environment template."""
    return f"""# Generated by Zenith CLI on {timestamp}
# Local Development Environment

# Application Configuration
SECRET_KEY=your-secret-key-here  # Generate with: zen keygen
...
def _generate_heroku_env_template(timestamp: str) -> str:
    """Generate Heroku deployment environment template."""
    return f"""# Generated by Zenith CLI on {timestamp}
# Heroku Deployment Environment

# Application Configuration
SECRET_KEY=your-production-secret-key  # Generate with: zen keygen --env production
ZENITH_ENV=production
DEBUG=false

# Database Configuration (automatically provided by Heroku Postgres)
# DATABASE_URL=postgres://...  # Set automatically by Heroku

# Redis Configuration (Heroku Redis addon)
# REDIS_URL=redis://...  # Set automatically by Heroku Redis

# Web Configuration
PORT=8000  # Required by Heroku
WEB_CONCURRENCY=4  # Number of worker processes

# CORS Configuration
CORS_ORIGINS=https://yourapp.herokuapp.com

# Email Configuration
# EMAIL_URL=smtp://smtp.sendgrid.net:587
# SENDGRID_API_KEY=SG...

# Storage Configuration (use AWS S3 or similar)
...
def _generate_railway_env_template(timestamp: str) -> str:
    """Generate Railway deployment environment template."""
    return f"""# Generated by Zenith CLI on {timestamp}
...
# Database Configuration (automatically provided by Railway Postgres)
# DATABASE_URL=postgresql://...  # Set automatically by Railway

# Redis Configuration (Railway Redis addon)
# REDIS_URL=redis://...  # Set automatically by Railway Redis

# Web Configuration
PORT=8000  # Railway can auto-detect, but explicit is better
HOST=0.0.0.0
...
def _generate_docker_env_template(timestamp: str) -> str:
    """Generate Docker deployment environment template."""
    return f"""# Generated by Zenith CLI on {timestamp}
...
# DATABASE_URL=sqlite:///./data/app.db  # For volume-mounted SQLite
...
# Email Configuration
EMAIL_URL=smtp://mailhog:1025  # For development
# EMAIL_URL=smtp://smtp.sendgrid.net:587  # For production

# Storage Configuration
STORAGE_URL=file:///app/uploads  # Container volume
# AWS_ACCESS_KEY_ID=your-access-key
# AWS_SECRET_ACCESS_KEY=your-secret-key
# AWS_STORAGE_BUCKET_NAME=your-bucket
...
def _generate_aws_env_template(timestamp: str) -> str:
    """Generate AWS deployment environment template."""
    return f"""# Generated by Zenith CLI on {timestamp}
# AWS Deployment Environment (ECS/Lambda/Elastic Beanstalk)

# Application Configuration
SECRET_KEY=your-production-secret-key  # Store in AWS Secrets Manager
ZENITH_ENV=production
DEBUG=false

# Database Configuration (RDS)
DATABASE_URL=postgresql://user:password@your-rds-endpoint:5432/dbname
# Use AWS Secrets Manager: arn:aws:secretsmanager:region:account:secret:name
...
AWS_ACCESS_KEY_ID=your-access-key  # Use IAM roles instead when possible
AWS_SECRET_ACCESS_KEY=your-secret-key  # Use IAM roles instead when possible

# S3 Storage Configuration
AWS_STORAGE_BUCKET_NAME=your-app-bucket
AWS_S3_REGION_NAME=us-east-1
AWS_S3_CUSTOM_DOMAIN=your-app-bucket.s3.amazonaws.com
...
# CloudWatch Logging
AWS_CLOUDWATCH_LOG_GROUP=/aws/ecs/your-app
AWS_CLOUDWATCH_LOG_STREAM=your-app-stream
...
def _generate_gcp_env_template(timestamp: str) -> str:
    """Generate Google Cloud Platform deployment environment template."""
    return f"""# Generated by Zenith CLI on {timestamp}
# Google Cloud Platform Deployment Environment

# Application Configuration
SECRET_KEY=your-production-secret-key  # Store in Secret Manager
ZENITH_ENV=production
DEBUG=false
...
GCS_BUCKET_NAME=your-app-bucket
GCS_PROJECT_ID=your-project-id

# Cloud Firestore Configuration (optional)
FIRESTORE_PROJECT_ID=your-project-id

# Email Configuration (SendGrid or Cloud Functions)
# SENDGRID_API_KEY=SG...
# CLOUD_FUNCTIONS_EMAIL_URL=https://region-project.cloudfunctions.net/sendEmail
...
# Cloud Run Specific
# CLOUD_RUN_SERVICE=your-service
