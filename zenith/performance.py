...
async def _store_in_cache(cache_key: str, result: Any, expires_at: float, cached_at: float) -> None:
...
def get_cache_info() -> dict[str, Any]:
    ...
    current_time = time.time()
    expired_count = sum(
        1 for item in _function_cache.values()
...
def query_cache_key(model_name: str, filters: dict | None = None, order_by: str | None = None) -> str:
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if duration_ms > threshold_ms:
                    logger.warning(f"Slow query: {func.__name__} took {duration_ms:.1f}ms")
        return wrapper
    return decorator
