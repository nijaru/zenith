...
class TestClient:
    ...
    async def request(
        ...
        **kwargs
        ...
        if not self._client:
            raise RuntimeError("TestClient not initialized. Use 'async with' context manager.")
    ...
    def sync_request(self, method: str, url: str, **kwargs) -> httpx.Response:
        ...
        if asyncio.get_running_loop():
            raise RuntimeError("Cannot use sync_request in async context. Use request() instead.")
...
class SyncTestClient:
    ...
    def set_auth_token(self, email: str, user_id: Union[int, str] = 1, role: str = "user") -> str:
    ...
    def request(self, method: str, url: str, headers: Optional[Dict[str, str]] = None, **kwargs):
        """Make HTTP request."""
        prepared_headers = self._prepare_headers(headers)
        return self._starlette_client.request(method, url, headers=prepared_headers, **kwargs)
